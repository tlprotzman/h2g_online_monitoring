...
channel_stream::channel_stream(int fpga_id, int asic_id, int channel, TH2 *adc_per_channel, TH2 *tot_per_channel, TH2 *toa_per_channel) {
    ...
    adc_spectra = new TH1D(Form("adc_spectra_%d_%d_%d", fpga_id, asic_id, channel), Form("ADC Spectra FPGA %d ASIC %d Channel %d", fpga_id, asic_id, channel), 256, 0, 1024);
    ...
    adc_pedestal = new TH1D(Form("adc_pedestal_%d_%d_%d", fpga_id, asic_id, channel), Form("ADC Pedestal FPGA %d ASIC %d Channel %d", fpga_id, asic_id, channel), 300, 0, 300);
    adc_max_minus_ped = new TH1D(Form("adc_max_minus_ped_%d_%d_%d", fpga_id, asic_id, channel), Form("ADC Max FPGA %d ASIC %d Channel %d", fpga_id, asic_id, channel), 1124, -100, 1024);   

    adc_spectra->SetTitle("");
    tot_spectra->SetTitle("");
    toa_spectra->SetTitle("");

    adc_pedestal->SetTitle("");
    adc_max_minus_ped->SetTitle("");
...
void channel_stream::construct_event(uint32_t timestamp, uint32_t adc) {
    ...
    if (current_event->is_complete()) {
        ...
        uint32_t max = current_event->get_max_sample();
        uint32_t ped = current_event->get_pedestal();
        fill_ped_and_max(ped, max);
...
void channel_stream::fill_ped_and_max(uint32_t pedestal, uint32_t max) {
    adc_pedestal->Fill(pedestal);
    adc_max_minus_ped->Fill(max - pedestal);
}
...
void channel_stream::reset() {
    ...
    adc_max->Reset("ICESM");
    adc_pedestal->Reset("ICESM");
    adc_max_minus_ped->Reset("ICESM");
}